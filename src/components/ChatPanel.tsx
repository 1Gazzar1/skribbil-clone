...
export function ChatPanel({ onGuess }: { onGuess?: (guess: string) => void }) {
    ...
    <div className="flex flex-col h-full bg-white rounded-sm md:rounded-3xl border border-blue-200 playful-shadow overflow-hidden">
        ...
        {messages.map((msg) => (
          <div
            ...
            className={`px-2 py-1.5 md:p-3 rounded-xl md:rounded-2xl max-w-[92%] ${
              msg.type === "system"
                ? "bg-amber-100 text-amber-800 font-bold self-center text-center w-full text-[10px] md:text-sm rounded-lg py-1"
                : msg.sender === "You"
          ...
          >
            {msg.type !== "system" && (
              <span className={`font-bold text-[10px] md:text-xs block mb-0.5 opacity-80 ${msg.sender === "You" ? "text-blue-100" : "text-slate-500"}`}>
                {msg.sender}
              </span>
            )}
            <span className={`font-semibold ${msg.type === "system" ? "text-[10px] md:text-sm" : "text-xs md:text-base"}`}>
              {msg.text}
            </span>
          </div>
        ))}
      ...
      <form onSubmit={handleSend} className="p-1.5 md:p-4 bg-blue-50 border-t-2 border-blue-100 flex gap-1.5 md:gap-2 shrink-0">
        <input
          type="text"
          placeholder="Guess..."
          className="flex-1 rounded-lg md:rounded-xl border-2 border-blue-200 bg-white text-slate-800 px-2 md:px-3 py-1 md:py-2 font-semibold text-xs md:text-base focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
          value={input}
          onChange={(e) => setInput(e.target.value)}
        />
        <button
          type="submit"
          className="bg-primary text-white p-1.5 md:p-3 rounded-lg md:rounded-xl hover:bg-blue-600 transition-colors shadow-sm flex items-center justify-center shrink-0"
        >
